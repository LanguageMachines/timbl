...
namespace Timbl {
  ...
  FeatureValue *Feature_s::add_value( size_t index,
  ...
  FeatureValue *Feature_s::add_value( size_t index,
    ...
    if (  it == features[index]->reverse_values.end() ){
      ...
      fv->ValFreq( freq );
      features[index]->reverse_values[hash_val] = fv;
      features[index]->values_array.push_back( fv );
    }
    else {
      it->second->IncValFreq( freq );
    }
    FeatureValue *result = features[index]->reverse_values[hash_val];
    if ( tv ){
      result->TargetDist.IncFreq(tv, freq );
    }
    return result;
